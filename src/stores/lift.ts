...
import { shallowRef, ref, reactive, markRaw, onUnmounted } from 'vue' 
    ...
    private moveToNextTarget() {
        ...
        const moveInterval = setInterval(() => {
            if (this.nowFloor.value < nextTarget) {
                ...
                const filtered = this.disabledButtons.filter(f => f !== this.nowFloor.value);
...
export const useLiftStore = defineStore('liftStore', () => {
    ...
    const openLiftDoor = (liftId: number) => {
        const lift = getLiftById(liftId);
        if (lift && !lift.isMoving.value) { // 移动中不能开门
            lift.open();
        }
    };
   ...
   function assignTask(floor: number, direction: 'up' | 'down') {
    ...
    const idleLifts = lifts.value.filter(lift => 
        lift.direction.value === 'idle' && !lift.isMoving.value
    );
    
    if (idleLifts.length > 0) {
        console.log('分配给空闲电梯:', idleLifts[0].id); // 新增
        // 选择距离最近的空闲电梯
        idleLifts.sort((a, b) => Math.abs(a.nowFloor.value - floor) - Math.abs(b.nowFloor.value - floor));
    ...
    if (sameDirectionLifts.length > 0) {
        ...
        sameDirectionLifts.sort((a, b) => 
            Math.abs(a.nowFloor.value - floor) - Math.abs(b.nowFloor.value - floor)
        );
